...
<head>
  ...
  <title>Study Manager — Quản lý học tập cá nhân (Mở rộng)</title>
  <style>
    :root{
...
<body>
  <div class="container">
    <header>
      <div class="brand">
        ...
        <div>
          <h1>Study Manager</h1>
          <p>Quản lý học tập cá nhân — lên kế hoạch, thời khóa biểu, theo dõi tiến độ</p>
        </div>
      </div>
      <nav>
    ...
    <main class="app">
      ...
      <aside class="card sidebar">
        <h2>Tạo nhiệm vụ nhanh</h2>
        <div class="add-task">
          <input id="task-title" type="text" placeholder="Tên nhiệm vụ (ví dụ: Ôn Toán chương 2)" />
          <div class="row">
          ...
          <div class="row">
            <input id="task-est" type="text" placeholder="Thời lượng (30 phút)" />
            <button id="add-task">Thêm</button>
          </div>
        </div>

        <h2 style="margin-top:12px">Danh sách nhiệm vụ</h2>
        <div id="task-list" class="task-list">
          <div class="empty">Chưa có nhiệm vụ — thêm nhiệm vụ ở trên</div>
        </div>

        <hr style="margin:12px 0;border:0;border-top:1px dashed rgba(255,255,255,0.03)" />

        <h2>Thời khóa biểu (nhanh)</h2>
        <div class="small-muted">Thêm môn cho từng thứ</div>
        <div style="margin-top:8px;display:flex;flex-direction:column;gap:8px">
          ...
          <input id="tkb-subject" placeholder="Tên môn (ví dụ: Toán)" />
          <div class="row">
            <input id="tkb-time" type="time" />
            <button id="add-tkb">Thêm</button>
          </div>
          ...
          <div class="planner">
            <div>
              ...
              <div style="margin-top:12px;border-top:1px dashed rgba(255,255,255,0.03);padding-top:12px">
                ...
                <input id="plan-title" type="text" placeholder="Việc cần làm" />
                <div class="row" style="margin-top:8px">
                  <input id="plan-time" type="time" />
                  <button id="add-plan">Thêm vào kế hoạch</button>
                </div>
              </div>
            </div>
            ...
            <div>
              <h2>Ghi chú nhanh</h2>
              <div class="notes">
                <textarea id="notes-area" placeholder="Ghi chú, công thức, ý tưởng..."></textarea>
                <div style="display:flex;gap:8px;margin-top:8px;justify-content:space-between">
                  <div class="muted">Lưu tự động vào trình duyệt</div>
                  <div>
                    <button id="clear-notes" class="btn-ghost">Xóa</button>
                    <button id="save-notes">Lưu</button>
                  </div>
                </div>
              </div>
              ...
              <h2>Bộ hẹn giờ (Pomodoro)</h2>
              <div class="timer">
                <div class="big-time" id="timer-display">25:00</div>
                <div class="muted">Chu kì: <span id="timer-mode">Làm việc</span></div>
                <div class="timer-controls">
                  <button id="start-timer">Bắt đầu</button>
                  <button id="pause-timer" class="btn-ghost">Tạm dừng</button>
                  <button id="reset-timer" class="btn-ghost">Đặt lại</button>
                </div>
              ...
              <div class="small-muted">Đặt thời gian cho từng môn — tổng phải từ 1h (60 phút) đến 4h (240 phút)</div>
              <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">
                <input id="study-subject" placeholder="Tên môn (Ví dụ: Toán)" />
                <input id="study-min" type="number" placeholder="Phút" min="5" style="width:120px" />
                <button id="add-study-time">Thêm</button>
              </div>
              <div id="study-plan" style="margin-top:8px" class="small-muted"></div>
              <div style="display:flex;gap:8px;margin-top:8px">
                <button id="start-study">Bắt đầu học</button>
                <button id="stop-study" class="btn-ghost">Dừng</button>
              </div>
              <div id="study-timer" style="margin-top:10px" class="muted">Chưa có phiên học</div>
  ...
  <script>
    ...
    function addTask(title, date, priority, est){
      if(!title) return alert('Hãy nhập tên nhiệm vụ.');
    ...
    function deleteTask(id){ if(!confirm('Xóa nhiệm vụ này?')) return; tasks = tasks.filter(t=>t.id!==id); saveTasks(); }
    function editTask(id){
      const t = tasks.find(x=>x.id===id); if(!t) return;
      const newTitle = prompt('Sửa tên nhiệm vụ', t.title); if(newTitle!==null){ t.title=newTitle; saveTasks(); }
    }
    function renderTasks(filter='all'){
      ...
      if(shown.length===0){ taskListEl.innerHTML='<div class="empty">Không có nhiệm vụ phù hợp</div>'; return }
      ...
      for(const t of shown){
        ...
        info.innerHTML=`<div class="title">${escapeHtml(t.title)}</div><small>${t.date? 'Hạn: '+t.date : ''} ${t.est? ' • '+escapeHtml(t.est): ''}</small>`;
        const icons = document.createElement('div'); icons.className='icons';
        const btnEdit = document.createElement('button'); btnEdit.className='icon-btn'; btnEdit.innerText='✏️'; btnEdit.title='Sửa'; btnEdit.addEventListener('click',()=>editTask(t.id));
        const btnDel = document.createElement('button'); btnDel.className='icon-btn'; btnDel.innerText='🗑️'; btnDel.title='Xóa'; btnDel.addEventListener('click',()=>deleteTask(t.id));
    ...
    function addPlan(title, date, time){ if(!title) return alert('Nhập nội dung kế hoạch.'); plans.push({id:Date.now(),title,date,time}); savePlans(); qs('#plan-title').value=''; }
    function deletePlan(id){ if(!confirm('Xóa kế hoạch?')) return; plans = plans.filter(p=>p.id!==id); savePlans(); }
    function renderPlanner(date){
      const el = qs('#planner-list'); el.innerHTML='';
      const list = plans.filter(p=>p.date===date);
      if(list.length===0) { el.innerHTML='<div class="empty">Chưa có kế hoạch cho ngày này</div>'; return }
      list.sort((a,b)=> (a.time||'') > (b.time||'') ? 1:-1);
      for(const p of list){
        const d = document.createElement('div'); d.className='task';
        d.innerHTML=`<div><div class="title">${escapeHtml(p.title)}</div><small>${p.time||''}</small></div>`;
        const icons=document.createElement('div'); icons.className='icons';
        const del=document.createElement('button'); del.className='icon-btn'; del.innerText='🗑️'; del.title='Xóa'; del.addEventListener('click',()=>deletePlan(p.id));
    ...
    function saveNotes(){ notes = notesArea.value; localStorage.setItem(KEY_NOTES, notes); showBanner('Ghi chú đã được lưu', 2000); }
    qs('#save-notes').addEventListener('click', saveNotes);
    qs('#clear-notes').addEventListener('click', ()=>{ if(confirm('Xóa toàn bộ ghi chú?')){ notesArea.value=''; saveNotes(); }});
    ...
    function startTimer(){ if(isRunning) return; isRunning=true; clearInterval(timerInterval); timerInterval=setInterval(()=>{ if(timerSeconds>0){ timerSeconds--; updateTimerDisplay(); } else { clearInterval(timerInterval); isRunning=false; isWork=!isWork; timerMode.innerText = isWork? 'Làm việc':'Nghỉ'; timerSeconds = (isWork? parseInt(qs('#work-min').value||25) : parseInt(qs('#break-min').value||5))*60; updateTimerDisplay(); beep(); notify('Study Manager', isWork? 'Bắt đầu chu kì làm việc':'Bắt đầu nghỉ ngắn'); } },1000); }
    function pauseTimer(){ if(timerInterval) clearInterval(timerInterval); isRunning=false; }
    function resetTimer(){ pauseTimer(); isWork=true; timerSeconds = parseInt(qs('#work-min').value||25)*60; timerMode.innerText='Làm việc'; updateTimerDisplay(); }
    ...
    function renderWeekChart(){ const el = qs('#week-chart'); el.innerHTML=''; const today = new Date();
      for(let i=6;i>=0;i--){ const d=new Date(); d.setDate(today.getDate()-i); const iso = d.toISOString().slice(0,10); const count = tasks.filter(t=>t.date===iso && t.done).length;
        const bar=document.createElement('div'); bar.style.height = Math.min(80,10+count*12)+'px'; bar.style.background='linear-gradient(180deg,var(--accent),var(--accent-2))'; bar.style.borderRadius='8px'; bar.title = d.toLocaleDateString()+' — Hoàn thành: '+count; el.appendChild(bar);
      }
    }
    ...
    qs('#add-tkb').addEventListener('click', ()=>{
      ...
      if(!subj) return alert('Nhập tên môn');
      if(!tkb[day]) tkb[day]=[];
    ...
    function removeTKB(day,id){
      if(!confirm('Xóa môn này khỏi thời khóa biểu?')) return;
      tkb[day] = (tkb[day]||[]).filter(x=>x.id!==id); saveTKB();
    }
    function renderTKBList(){
      tkbListEl.innerHTML='';
      const keys = Object.keys(tkb).sort();
      if(keys.length===0){ tkbListEl.innerHTML='<div class="small-muted">Chưa có thời khóa biểu</div>'; return; }
      for(const k of keys){
        ...
        for(const it of items){
          ...
          row.innerHTML = `<div class="small-muted">${escapeHtml(it.subj)} ${it.time? ' • '+it.time:''}</div>`;
          const del = document.createElement('button'); del.className='icon-btn'; del.innerText='🗑️'; del.addEventListener('click',()=>removeTKB(k,it.id));
          row.appendChild(del);
    ...
    function renderTKBFull(){
      ...
      for(let d=1; d<=6; d++){
        ...
        box.innerHTML = `<div style="font-weight:700">${dayName(d)}</div><div class="small-muted">${arr.length? arr.map(x=>escapeHtml(x.subj)+(x.time? ' • '+x.time:'')).join('<br>') : 'Không có'}</div>`;
        tkbFullEl.appendChild(box);
      }
      const sun = tkb[0]||[]; const sunBox=document.createElement('div'); sunBox.style.marginBottom='8px';
      sunBox.innerHTML = `<div style="font-weight:700">${dayName(0)}</div><div class="small-muted">${sun.length? sun.map(x=>escapeHtml(x.subj)+(x.time? ' • '+x.time:'')).join('<br>') : 'Không có'}</div>`;
    ...
    qs('#add-study-time').addEventListener('click', ()=>{
      ...
      if(!subj || !mins) return alert('Nhập môn và số phút');
      ...
      if(total < 60 || total > 240){
        showBanner('⚠ Tổng thời gian hiện tại: '+total+' phút — yêu cầu 60–240 phút để bắt đầu phiên học', 4000, true);
      }
      saveStudyPlan();
    });
    ...
    function renderStudyPlanUI(){
      ...
      if(studyPlan.length===0) { el.innerHTML='<div class="empty">Chưa có lịch học</div>'; return; }
      let total = 0;
      for(const s of studyPlan){
        total += s.mins;
        const row = document.createElement('div'); row.style.display='flex'; row.style.justifyContent='space-between'; row.style.alignItems='center';
        row.innerHTML = `<div>${escapeHtml(s.subj)} — ${s.mins} phút</div>`;
        const del = document.createElement('button'); del.className='icon-btn'; del.innerText='🗑️'; del.addEventListener('click',()=>removeStudyItem(s.id));
        row.appendChild(del);
        el.appendChild(row);
      }
      const sum = document.createElement('div'); sum.className='small-muted'; sum.style.marginTop='6px'; sum.innerText = 'Tổng: '+total+' phút';
      el.appendChild(sum);
      if(total < 60 || total > 240){
        const warn = document.createElement('div'); warn.className='danger'; warn.style.marginTop='6px'; warn.innerText='Tổng thời gian phải từ 60 đến 240 phút để bắt đầu.';
        el.appendChild(warn);
      }
    }
    ...
    function startStudySession(){
      const total = studyPlan.reduce((s,x)=>s+x.mins,0);
      if(total < 60 || total > 240) return alert('Tổng thời gian phải từ 60 đến 240 phút trước khi bắt đầu phiên học.');
      if(studyPlan.length===0) return alert('Chưa có môn học trong lịch!');
      studyIndex = 0;
      startNextSubject();
    }
    function startNextSubject(){
      if(studyIndex >= studyPlan.length){ showBanner('✅ Hoàn thành phiên học!', 3000); qs('#study-timer').innerText = 'Đã hoàn thành phiên học.'; notify('Study Manager', 'Phiên học đã hoàn thành'); return; }
      const cur = studyPlan[studyIndex];
      studySeconds = cur.mins * 60;
      qs('#study-timer').innerText = `Đang học ${cur.subj} — ${cur.mins} phút`;
      renderSupportLinks(cur.subj);
      notify('Study Manager', 'Bắt đầu học: '+cur.subj+' — '+cur.mins+' phút');
      beep();
      clearInterval(studyInterval);
      studyInterval = setInterval(()=>{
        studySeconds--;
        const mm = Math.floor(studySeconds/60); const ss = studySeconds%60;
        qs('#study-timer').innerText = `Đang học ${cur.subj} — ${mm.toString().padStart(2,'0')}:${ss.toString().padStart(2,'0')}`;
        if(studySeconds<=0){
          clearInterval(studyInterval);
          beep();
          notify('Study Manager', 'Hoàn thành môn: '+cur.subj);
          showBanner('⏰ Hết thời gian: '+cur.subj, 2500);
          studyIndex++;
    ...
    function stopStudySession(){
      ...
      studyInterval = null;
      qs('#study-timer').innerText = 'Phiên học đã dừng.';
      showBanner('Phiên học đã dừng', 1500);
    }
    ...
    function renderSupportLinks(subj){
      ...
      if(links.length===0){ container.innerHTML = `<div class="small-muted">Không có link hỗ trợ cho ${escapeHtml(subj)} — bạn có thể thêm trong code.</div>`; return; }
      let html = `<div style="font-weight:700">Tài liệu hỗ trợ cho ${escapeHtml(subj)}:</div>`;
    ...
    qs('#add-eval').addEventListener('click', ()=>{
      ...
      if(!t || !d) return alert('Nhập tên bài tập và hạn nộp.');
      evals.push({id:Date.now(), title:t, date:d, status:'chưa'});
      qs('#eval-title').value=''; qs('#eval-date').value='';
      saveEvals();
    });
    function updateEval(id, val){ evals = evals.map(e=> e.id===id? {...e, status: val }: e ); saveEvals(); }
    function deleteEval(id){ if(!confirm('Xóa bài tập này?')) return; evals = evals.filter(e=>e.id!==id); saveEvals(); }
    function renderEvals(){
      const el = qs('#eval-list'); el.innerHTML='';
      if(evals.length===0){ el.innerHTML='<div class="small-muted">Chưa có bài tập</div>'; return; }
      const today = nowISODate();
      for(const e of evals){
        ...
        const left = document.createElement('div'); left.innerHTML = `<div style="font-weight:700">${escapeHtml(e.title)}</div><div class="small-muted">Hạn: ${e.date}${e.date===today? ' • Hạn hôm nay!':''}</div>`;
        const right = document.createElement('div');
        const sel = document.createElement('select'); sel.innerHTML = `<option value="chưa">chưa</option><option value="đang">đang</option><option value="xong">xong</option>`; sel.value = e.status; sel.addEventListener('change', ()=>updateEval(e.id, sel.value));
        const del = document.createElement('button'); del.className='icon-btn'; del.innerText='🗑️'; del.addEventListener('click', ()=>deleteEval(e.id));
        ...
        if(diff <= 1 && diff >= 0 && e.status !== 'xong'){
          const warn = document.createElement('div'); warn.className='danger'; warn.style.fontSize='12px'; warn.innerText = '⚠ Gần đến hạn nộp!';
          row.appendChild(warn);
          showBanner('⚠ Bài tập gần hạn: '+e.title, 3000, true);
        }
      }
    }
    ...
    function startUpScheduleNotice(){
      ...
      if(arr.length > 0){
        showBanner(`📅 Lịch ${dayName(showDay)}: ${arr.map(x=>x.subj).join(', ')}`, 7000);
        notify('Study Manager', `Lịch ${dayName(showDay)}: ${arr.map(x=>x.subj).join(', ')}`);
      } else {
        const days = [1,2,3,4,5,6,0];
        for(const dd of days){
          if((tkb[dd]||[]).length > 0){
            showBanner(`📅 Lịch sắp tới (${dayName(dd)}): ${(tkb[dd]||[]).map(x=>x.subj).join(', ')}`, 7000);
            break;
          }
        }
      }
